            ...
            with open(self._csv_file, mode='a+') as csv_f:
                ...
                if self._field_names is None:
                    ...
                    if not np.array_equal(self._field_names, self._row_data.keys()):
                        ...
                        missing_keys = list(set(self._field_names) - set(self._row_data.keys()))
                        if len(missing_keys)>0:
                            for mk in missing_keys:
                                self._row_data[mk] = self._prev_row_data[mk]

                        elif len(list(set(self._row_data.keys()) - set(self._field_names))) >0:
                            writer.fieldnames = self._row_data.keys()
